...
static ssize_t leds_read(struct file *file, char *buffer, size_t len, loff_t *offset)
{
	 ...
	 * "file"s private data to the appropriate miscdevice structure. We then use
	 * the container_of macro to get the structure that miscdevice is stored
	 * inside of (which is our custom_leds_dev structure that has the current led
	 * value).
	 *
	...
	if(success != 0) {
		dev_dbg(&dev->pdev->dev, "Failed to return current led value to userspace\n");
...
static ssize_t leds_write(struct file *file, const char *buffer, size_t len, loff_t *offset)
{
	...
	*/
	...
	if(success != 0) {
		dev_dbg(&dev->pdev->dev, "Failed to read led value from userspace\n");
		return -EFAULT; // Bad address error value. It's likely that "buffer" doesn't point to a good address
	} else {
...
static int custom_leds_probe(struct platform_device *pdev)
{
	int ret_val = -EBUSY;
	struct custom_leds_dev *dev;
	struct resource *r = 0;

	dev_dbg(&pdev->dev, "leds_probe enter\n");
	...
	dev_dbg(&pdev->dev, "leds_probe exit\n");
...
bad_exit_return:
	dev_err(&pdev->dev, "leds_probe bad exit :(\n");
	return ret_val;
}
...
static int custom_leds_remove(struct platform_device *pdev)
{
	// Grab the instance-specific information out of the platform device
	struct custom_leds_dev *dev = 
		(struct custom_leds_dev*)platform_get_drvdata(pdev);

	dev_dbg(&pdev->dev, "leds_remove enter\n");
	...
	dev_dbg(&pdev->dev, "leds_remove exit\n");
...
static struct platform_driver custom_leds = {
	.probe = custom_leds_probe,
	.remove = custom_leds_remove,
	.driver = {
...
module_platform_driver(custom_leds)

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Cameron Kirk");
MODULE_AUTHOR("Dario Mylonopoulos");
MODULE_DESCRIPTION("A simple Linux driver for a custom led device.");
MODULE_VERSION("1.0");
